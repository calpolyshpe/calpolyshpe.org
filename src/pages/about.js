    ...
    return (
        <main>
            ...
            <div className="bg-[#001F5B] flex justify-center items-center">
                {SHPEvalues.map((value, index) => (
                    <div key={index} className="lg:w-3/5 w-3/4 h-auto">
                        <h1 className="text-white flex justify-center lg:text-2xl md:text-xl text-[13px] font-medium font-sans md:mt-6 mt-3">{value.pilar}</h1>
                        <h2 className="text-white flex justify-center lg:text-xl md:text-sm text-[9px] font-sans md:mt-6 mt-3" >{value.body}</h2>
                    </div>
                ))}
